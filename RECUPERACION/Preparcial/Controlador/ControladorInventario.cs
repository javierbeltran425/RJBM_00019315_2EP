            ...
            try
            {
                ...
                foreach(DataRow dr in dt.Rows)
                {
                    productos.Add(new Inventario
                        (
                            Convert.ToInt32(dr[0].ToString()), //Conversión del dato recibido a tipo int
                            dr[1].ToString(),
                            dr[2].ToString(),
                            Convert.ToDecimal(dr[3].ToString()), // Conversion del dato recibido a tipo Double
                            Convert.ToInt32(dr[4].ToString()) // Conversión del dato recibido a tipo int
                        )
                    );
                }
            }
